...
export default function Page({ params }) {
  ...
  const postFeedback = async (feedbackData) => {
    let response = await fetch(
      "https://review-reflection.vercel.app/store/feedback",
      {
        method: "POST",
        headers: {
  ...
  return storeData?.statusCode == "404" ? (
    redirect("/not-found")
  ) : (
    <section>
                  ...
                  case "single integer":
                    return (
                      <StarRating
                        ...
                        ratingCallBack={(ratingData) =>
                          handleRatingValue(item.question_store_id, ratingData)
                        }
                      />
                    );
                  case "Boolean":
                    return (
                      <Radio
                        ...
                        key={id}
                        radioData={radioData}
                        radioCallBack={(radioData) =>
                          handleRadioValue(item.question_store_id, radioData)
                        }
                      />
                    );
                  case "string":
                    return (
                      <InputText
                        ...
                        sendDataToParent={(textData) =>
                          handleTextValue(item.question_store_id, textData)
                        }
                      />
                    );
                  case "paragraph":
